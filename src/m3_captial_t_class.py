...
class CapitalT(object):
    ...
    def __init__(self, intersection_center, width, height, letter_thickness):
        ...
        # --------------------------------------------------------------
        horz_x = intersection_center.x - (0.5 * width)
        horz_y = intersection_center.y
        vert_x = intersection_center.x - (0.5 * letter_thickness)
        vert_y = intersection_center.y

        self.h_rect = rg.Rectangle(rg.Point(horz_x, horz_y), rg.Point(horz_x + width, horz_y + letter_thickness))
        self.v_rect = rg.Rectangle(rg.Point(vert_x, vert_y), rg.Point(vert_x + letter_thickness, vert_y + height))
    ...
    def clone(self):
        ...
        #     clone.pdf.
        # --------------------------------------------------------------
        a1 = self.h_rect.get_center()
        a2 = self.h_rect.get_width()
        a3 = self.v_rect.get_height()
        a4 = self.h_rect.get_height()

        t1 = CapitalT(a1, a2, a3, a4)
        c1 = self.h_rect.fill_color
        c2 = self.v_rect.outline_color
        t1.set_colors(c1, c2)
